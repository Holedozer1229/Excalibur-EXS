// Admin worker: processes pending AETX claims by generating BRC-20 mint+transfer
// inscription payloads. Marks claims as `inscribing` for admin broadcast via UniSat/Xverse.
//
// POST /aetx-claims-worker
//   body (optional): { claim_ids?: string[], action?: "process" | "mark_inscribed" | "fail" | "cancel",
//                       inscription_id?: string, tx_hash?: string, note?: string, limit?: number }
...
/** BRC-20 mint limit per inscription (matches AETX deploy). */
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (args.action === "fail" || args.action === "cancel") {
      ...
      await admin.from("aetx_ledger").insert({
      ...
      const { data: updated } = await admin
        .from("aetx_claims")
        .update({
          status: args.action === "cancel" ? "canceled" : "failed",
          note: args.note ?? null,
        })
        .eq("id", claim.id)
        .select()
    ...
    const payloads = pending.map((c) => {
      ...
      const mint = { p: "brc-20", op: "mint", tick: TICK, amt };
      const transfer = { p: "brc-20", op: "transfer", tick: TICK, amt };
      const splitNote =
        c.amount > MINT_LIMIT
          ? ` Amount ${c.amount} exceeds mint limit ${MINT_LIMIT} — split into multiple mint inscriptions.`
          : "";
      return {
        claim_id: c.id,
        user_id: c.user_id,
        ...
        instructions:
          `1) Inscribe ${JSON.stringify(mint)} (mint up to ${MINT_LIMIT} per op), ` +
          `2) Inscribe ${JSON.stringify(transfer)} and send to ${c.btc_address}, ` +
          `3) POST action="mark_inscribed", claim_id="${c.id}", inscription_id="…".${splitNote}`,
      };
    });

    const ids = pending.map((c) => c.id);
    await admin.from("aetx_claims").update({ status: "inscribing" }).in("id", ids);
    ...
    console.error("aetx-claims-worker error:", e);
