...
# The thirteen words — published in PROTOCOL_QUICK_REFERENCE.md, never invented.
...
SEED_ITERATIONS = 600_000  # HPP-1, per repo spec
SEED_DKLEN = 32            # 256-bit anchor


def derive_seed(axiom: str = AXIOM) -> bytes:
    """256-bit sovereign anchor seed. Public, deterministic, verifiable."""
    return hashlib.pbkdf2_hmac("sha512", axiom.encode("utf-8"),
                               SEED_SALT, SEED_ITERATIONS, SEED_DKLEN)


def domain_axiom(axiom: str, seed: bytes) -> str:
    """Seed-domain-separated mining preimage root."""
    return "%s:%s" % (axiom, seed.hex())
...
def _lane_worker(args):
    ...
    while True:
        ...
        results = kernel.fused_hash_computation(daxiom, start, chunk,
                                                rounds=128)
...
def mine_parallel(daxiom: str, difficulty: int, lanes: int,
                  batch_size: int = 32):
...
def mine(kernel: UniversalMiningKernel, daxiom: str, difficulty: int,
         nonce_start: int = 0, max_attempts: int = 1 << 63,
    ...
    while total < max_attempts:
        ...
        results = kernel.fused_hash_computation(daxiom, nonce, batch,
                                                rounds=128)
        for n, final_hash, _ in results:
            if final_hash[:difficulty] == target:
                dt = time.time() - t0
        ...
        if total % log_every < batch:
            dt = time.time() - t0
            print("  ... %d hashes (%.1f kH/s)" % (total, total / dt / 1e3),
                  flush=True)
    raise RuntimeError("exhausted max_attempts=%d" % max_attempts)


def build_genesis(difficulty: int, batch_size: int = 32, lanes: int = 1):
    ...
    if lanes > 1:
        print("[*] grinding Ω′ Δ18 genesis @ difficulty %d zero bytes, "
              "%d lanes …" % (difficulty, lanes), flush=True)
        ...
        rate_note = "%.1f kH/s" % (hashes / dt / 1e3)
    ...
    return {
        ...
        "spec_refs": ["PROTOCOL_QUICK_REFERENCE.md",
                      "TETRA_POW_ENHANCEMENT.md", "ARCHITECTURE.md"],
        "height": 0,
        "prev": "00" * 32,
        "axiom": AXIOM,
        "axiom_words": 13,
...
def verify_descriptor(desc) -> bool:
    ok = True
    ...
    ok &= check("axiom is exactly 13 words", len(words) == 13,
                "(%d)" % len(words))
    ok &= check("axiom matches published taproot",
                desc["axiom"] == AXIOM)
    seed = derive_seed(desc["axiom"])
    ok &= check("seed == HPP-1(axiom)", seed.hex() ==
                desc["seed_derivation"]["seed_hex"])
...
def main(argv=None):
    ...
    if args.verify:
        ...
        print("descriptor: %s" % ("VALID sovereign genesis"
                                  if ok else "INVALID"), flush=True)
